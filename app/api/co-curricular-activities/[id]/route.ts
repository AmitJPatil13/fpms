...
export async function DELETE(
  ...
  try {
    ...
    const [deletedActivity] = await db.delete(coCurricularActivities)
      .where(
        and(
          eq(coCurricularActivities.id, id),
          eq(coCurricularActivities.userEmail, session.data.email)
        )
      )
      .returning();
    ...
    console.error('Failed to delete co-curricular activity:', error);
    return NextResponse.json(
